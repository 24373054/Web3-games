...
export class Web2WalletAdapter implements IWalletAdapter {
  ...
  getCurrentEpoch(): number {
    return this.mockWallet.getCurrentEpoch()
  }

  tryUnlockNextEpoch(): { success: boolean, message: string } {
    return this.mockWallet.tryUnlockNextEpoch()
  }

  checkKeywordAndReward(message: string, npcId?: string): { found: boolean, keyword?: string, fragmentId?: number } {
    return this.mockWallet.checkKeywordAndReward(message, npcId)
  }

  getKeywordHint(npcId?: string): string {
    return this.mockWallet.getKeywordHint(npcId)
  }

  getNpcStyle(npcId: string): string {
    return this.mockWallet.getNpcStyle(npcId)
  }

  getFoundKeywords(): string[] {
    return this.mockWallet.getFoundKeywords()
  }
}
...
export class Web3WalletAdapter implements IWalletAdapter {
  ...
  tryUnlockNextEpoch(): { success: boolean, message: string } {
    return { success: false, message: 'Web3模式请通过合约解锁纪元' }
  }
